...
mechanism covered it":
    ...
    D6 / D7   identity at one level and at two
    D8        the @cold ordering, which is the one drill whose baseline
              behaviour is GREEN - it is a regression gate, not an old red
...
GATES = ["tests/user-function-arity.test.ts", "tests/user-function-arity-identity.test.ts"]
os.chdir(ROOT)
...
print("control (v2, unmutated): %d failed | %d passed\n" % (ctrl_f, ctrl_p))
if ctrl_f != 0:
...
print("=" * 66)
print("```")
print("control (v2, unmutated)   %d failed | %d passed" % (ctrl_f, ctrl_p))
...
io.open(os.path.join(OUT, "drills-v2.json"), "w", encoding="utf-8", newline="\n").write(
