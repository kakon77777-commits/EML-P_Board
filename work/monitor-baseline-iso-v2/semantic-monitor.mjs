 ...
 *
 * Both path flags REQUIRE their path. Given without one, the run stops with
 * exit 2 and touches neither committed artifact — see the note above them.
 *
 * ── The ledger ────────────────────────────────────────────────────────────
 *
...
function refuse(problem) {
  ...
  console.error('  A path flag that cannot be resolved does not fall back to the');
  console.error('  committed artifact: sparing that artifact is what the redirect');
  console.error('  exists for. Nothing was read and nothing was written.');
  process.exit(2);
}
